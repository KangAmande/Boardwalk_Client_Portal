...
class PolicyMenu extends React.PureComponent<FilesProps> {
        ...
        return (
            <div>
                {this.props.File.map((d: FilesStore.Files) =>
                    <div>
                        <CustomAccordion key={d.id} title={"File " + d.name.toString()} content={<div><p>Created By {d.createdBy}</p></div>} />
                        <br />
                    </div>
                )}
            </div>
...
export default connect(
    ...
    FilesStore.actionCreators)(PolicyMenu as any);