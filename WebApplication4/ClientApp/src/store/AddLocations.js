...
exports.clientActionCreators = {
    requestClientAddLocations: function (startDateIndex) { return function (dispatch, getState) {
        // Only load data if it's something we don't already have (and are not already loading)
        var appState = getState();
...
var clientUnloadedState = { ClientAddLocation: [], isLoading: false };
...
var clientReducer = function (state, incomingAction) {
    ...
    switch (action.type) {
        case 'REQUEST_ADD_CLIENT_LOCATIONS':
            return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { ClientAddLocation: state.ClientAddLocation, isLoading: true });
        case 'RECEIVE_ADD_CLIENT_LOCATIONS':
            // Only accept the incoming data if it matches the most recent request. This ensures we correctly
            // handle out-of-order responses.
            if (action.startDateIndex === state.startDateIndex) {
                return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { ClientAddLocation: action.ClientAddLocation, isLoading: false });
            }
            break;
    }
    return state;
