...
exports.clientActionCreators = {
    requestClientAddEquipments: function (startDateIndex) { return function (dispatch, getState) {
        // Only load data if it's something we don't already have (and are not already loading)
        var appState = getState();
...
var clientUnloadedState = { ClientAddEquipment: [], isLoading: false };
...
var clientReducer = function (state, incomingAction) {
    ...
    switch (action.type) {
        case 'REQUEST_ADD_CLIENT_EQUIPMENTS':
            return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { ClientAddEquipment: state.ClientAddEquipment, isLoading: true });
        case 'RECEIVE_ADD_CLIENT_EQUIPMENTS':
            // Only accept the incoming data if it matches the most recent request. This ensures we correctly
            // handle out-of-order responses.
            if (action.startDateIndex === state.startDateIndex) {
                return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { ClientAddEquipment: action.ClientAddEquipment, isLoading: false });
            }
            break;
    }
    return state;
