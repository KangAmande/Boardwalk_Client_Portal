...
exports.clientActionCreators = {
    requestClientRemoveLocations: function (startDateIndex) { return function (dispatch, getState) {
        // Only load data if it's something we don't already have (and are not already loading)
        var appState = getState();
...
var clientUnloadedState = { ClientRemoveLocation: [], isLoading: false };
...
var clientReducer = function (state, incomingAction) {
    ...
    switch (action.type) {
        case 'REQUEST_REMOVE_CLIENT_LOCATIONS':
            return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { ClientRemoveLocation: state.ClientRemoveLocation, isLoading: true });
        case 'RECEIVE_REMOVE_CLIENT_LOCATIONS':
            // Only accept the incoming data if it matches the most recent request. This ensures we correctly
            // handle out-of-order responses.
            if (action.startDateIndex === state.startDateIndex) {
                return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { ClientRemoveLocation: action.ClientRemoveLocation, isLoading: false });
            }
            break;
    }
    return state;
