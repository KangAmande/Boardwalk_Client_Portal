...
var unloadedState = { Equipments: [], isLoading: false };
exports.reducer = function (state, incomingAction) {
    ...
    switch (action.type) {
        case 'REQUEST_EQUIPMENTS':
            return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { Equipments: state.Equipments, isLoading: true });
        case 'RECEIVE_EQUIPMENTS':
            // Only accept the incoming data if it matches the most recent request. This ensures we correctly
            // handle out-of-order responses.
            if (action.startDateIndex === state.startDateIndex) {
                return __assign(__assign({ startDateIndex: action.startDateIndex }, state), { Equipments: action.Equipments, isLoading: false });
            }
            break;
    }
    return state;
